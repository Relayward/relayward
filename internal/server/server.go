...
func New(options Options) http.Handler {
	...
	mux.HandleFunc("POST /api/v1/nodes/{node_id}/agent-updates/latest", server.withAuthentication(server.withCSRF(server.requestLatestAgentUpdate)))
	...
	mux.HandleFunc("GET /api/v1/plugin-services", server.withAuthentication(server.listPluginServices))
	mux.HandleFunc("GET /api/v1/announcement", server.withAuthentication(server.getAnnouncement))
	mux.HandleFunc("PUT /api/v1/announcement", server.withAuthentication(server.withCSRF(server.updateAnnouncement)))
	mux.HandleFunc("GET /api/v1/subscriptions/{subscription_token}", server.subscription)
	mux.HandleFunc("GET /api/v1/subscriptions/{subscription_token}/base64", server.downloadSubscription(store.SubscriptionFormatBase64, "text/plain; charset=utf-8", "relayward.txt"))
	mux.HandleFunc("GET /api/v1/subscriptions/{subscription_token}/mihomo.yaml", server.downloadSubscription(store.SubscriptionFormatMihomo, "application/yaml; charset=utf-8", "relayward-mihomo.yaml"))
	mux.HandleFunc("GET /api/v1/subscriptions/{subscription_token}/sing-box.json", server.downloadSubscription(store.SubscriptionFormatSingBox, "application/json; charset=utf-8", "relayward-sing-box.json"))
